...
.dantora-home .dh-hero{position:relative;min-height:min(890px,calc(100svh - 20px));overflow:hidden;border-radius:0 0 48px 48px;background:var(--mint)}.dantora-home .dh-hero__inner{display:grid;grid-template-columns:minmax(0,1.04fr) minmax(430px,.96fr);align-items:center;min-height:min(890px,calc(100svh - 20px));padding-block:70px 56px}.dantora-home .dh-hero__copy{position:relative;z-index:3;max-width:850px}.dantora-home .dh-hero h1{margin:25px 0 29px;max-width:900px;font-size:clamp(62px,7.5vw,132px);font-weight:540;line-height:.84;letter-spacing:-.075em}.dantora-home .dh-hero h1 em{font-family:Georgia,serif;font-weight:400}.dantora-home .dh-hero__bottom{display:flex;align-items:flex-end;gap:28px;max-width:700px}.dantora-home .dh-hero__bottom p{max-width:490px;margin:0;font-size:clamp(17px,1.4vw,21px);line-height:1.5}.dantora-home .dh-hero__scene{position:absolute;z-index:1;right:-3%;top:0;width:62%;height:100%;pointer-events:none}.dantora-home .dh-hero__scene canvas,.dantora-home .dh-contact__scene canvas{position:absolute;inset:0;width:100%;height:100%;display:block}.dantora-home .dh-dna{position:absolute;inset:3% 3% 3% auto;width:min(94%,680px);height:94%;overflow:visible;opacity:.92;filter:drop-shadow(0 22px 45px rgba(20,47,134,.18))}.dantora-home .dh-dna__body{transform-origin:center;animation:dh-dna-drift 9s ease-in-out infinite}.dantora-home .dh-dna__strand{fill:none;stroke-width:13;stroke-linecap:round;stroke-dasharray:28 10;animation:dh-dna-flow 4s linear infinite}.dantora-home .dh-dna__strand--b{animation-direction:reverse}.dantora-home .dh-dna__rungs line{stroke:#142f86;stroke-width:5;stroke-linecap:round;opacity:.46;animation:dh-dna-pulse 2.8s ease-in-out infinite}.dantora-home .dh-dna__rungs line:nth-child(2n){animation-delay:-.7s}.dantora-home .dh-dna__rungs line:nth-child(3n){animation-delay:-1.4s}.dantora-home .dh-dna__node{fill:#fff;stroke:#31b4f4;stroke-width:5;animation:dh-dna-pulse 2.4s ease-in-out infinite}.dantora-home .dh-dna__node--red{stroke:#da1c29;animation-delay:-1.2s}@keyframes dh-dna-flow{to{stroke-dashoffset:-76}}@keyframes dh-dna-drift{0%,100%{transform:translate3d(0,-1.5%,0) rotate(-5deg) scale(.98)}50%{transform:translate3d(-2.5%,1.5%,0) rotate(3deg) scale(1.04)}}@keyframes dh-dna-pulse{0%,100%{opacity:.32}50%{opacity:.95}}.dantora-home .dh-hero__orbit{position:absolute;right:4.5%;bottom:7%;z-index:2;width:112px;height:112px;border:1px solid rgba(8,47,39,.3);border-radius:50%;display:grid;place-items:center;font-size:11px;font-weight:800;letter-spacing:.12em;text-align:center;animation:dh-orbit 14s linear infinite}@keyframes dh-orbit{to{transform:rotate(360deg)}}
...
async function initClarusHome(root){
  ...
  }catch(error){console.warn('Three.js modules unavailable',error);state.sceneReady=true;assetBanner&&assetBanner.classList.add('is-visible');return;}
  ...
  const shaderNodes=['#dh-helix-v','#dh-helix-f','#dh-atmo-v','#dh-atmo-f'].map(selector=>document.querySelector(selector));if(shaderNodes.some(node=>!node)){state.sceneReady=true;assetBanner&&assetBanner.classList.add('is-visible');return;}const [vHelix,fHelix,vAtmo,fAtmo]=shaderNodes.map(node=>node.textContent||'');

  function seeded(i,salt){return (Math.sin(i*12.9898+salt*78.233)*43758.5453)%1;}
  function positive(v){return v<0?v+1:v;}
  function makeCloud(count,kind){const pos=new Float32Array(count*3);const seeds=new Float32Array(count);const kinds=new Float32Array(count);for(let i=0;i<count;i++){const r1=positive(seeded(i,kind+1)),r2=positive(seeded(i,kind+2)),r3=positive(seeded(i,kind+3));if(kind===0){const turn=r1*Math.PI*10.0;const strand=i%2===0?0:Math.PI;const radius=1.3+.12*Math.sin(turn*3.0);pos[i*3]=Math.cos(turn+strand)*radius;pos[i*3+1]=(r2-.5)*7.5;pos[i*3+2]=Math.sin(turn+strand)*radius;}else{const a=r1*Math.PI*2.0;const radius=Math.pow(r2,.55)*4.4;pos[i*3]=Math.cos(a)*radius;pos[i*3+1]=(r3-.5)*8.4;pos[i*3+2]=Math.sin(a)*radius*.72+(r2-.5)*1.5;}seeds[i]=r1;kinds[i]=kind;}const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.BufferAttribute(pos,3));g.setAttribute('aSeed',new THREE.BufferAttribute(seeds,1));g.setAttribute('aKind',new THREE.BufferAttribute(kinds,1));return g;}
  ...
  function createInkScene(container,isHero){
    ...
    const compact=innerWidth<768||navigator.hardwareConcurrency&&navigator.hardwareConcurrency<=4;const helixCount=compact?14000:32000;const inkCount=compact?42000:110000;
    const helix=new THREE.Points(makeCloud(helixCount,0),material);const ink=new THREE.Points(makeCloud(inkCount,1),material);helix.frustumCulled=false;ink.frustumCulled=false;scene.add(ink,helix);
...
function DnaBackdrop({ id }: { id: string }) {
  const blueGradient = `${id}-blue`;
  const redGradient = `${id}-red`;

  return (
    <svg className="dh-dna" viewBox="0 0 520 820" aria-hidden="true" focusable="false">
      <defs>
        <linearGradient id={blueGradient} x1="0" y1="0" x2="1" y2="1">
          <stop offset="0" stopColor="#142f86" />
          <stop offset="0.52" stopColor="#31b4f4" />
          <stop offset="1" stopColor="#142f86" />
        </linearGradient>
        <linearGradient id={redGradient} x1="1" y1="0" x2="0" y2="1">
          <stop offset="0" stopColor="#da1c29" />
          <stop offset="0.5" stopColor="#f16a74" />
          <stop offset="1" stopColor="#da1c29" />
        </linearGradient>
      </defs>
      <g className="dh-dna__body">
        <g className="dh-dna__rungs">
          <line x1="205" y1="72" x2="315" y2="72" />
          <line x1="145" y1="142" x2="375" y2="142" />
          <line x1="82" y1="220" x2="438" y2="220" />
          <line x1="92" y1="300" x2="428" y2="300" />
          <line x1="170" y1="378" x2="350" y2="378" />
          <line x1="170" y1="456" x2="350" y2="456" />
          <line x1="92" y1="534" x2="428" y2="534" />
          <line x1="82" y1="612" x2="438" y2="612" />
          <line x1="145" y1="690" x2="375" y2="690" />
          <line x1="205" y1="768" x2="315" y2="768" />
        </g>
        <path className="dh-dna__strand dh-dna__strand--a" stroke={`url(#${blueGradient})`} d="M130 -25 C470 35 470 135 130 195 C-70 255 -70 355 130 415 C470 475 470 575 130 635 C-70 695 -70 795 130 850" />
        <path className="dh-dna__strand dh-dna__strand--b" stroke={`url(#${redGradient})`} d="M390 -25 C50 35 50 135 390 195 C590 255 590 355 390 415 C50 475 50 575 390 635 C590 695 590 795 390 850" />
        <circle className="dh-dna__node" cx="205" cy="72" r="11" />
        <circle className="dh-dna__node dh-dna__node--red" cx="315" cy="72" r="11" />
        <circle className="dh-dna__node" cx="82" cy="220" r="11" />
        <circle className="dh-dna__node dh-dna__node--red" cx="438" cy="220" r="11" />
        <circle className="dh-dna__node" cx="170" cy="378" r="11" />
        <circle className="dh-dna__node dh-dna__node--red" cx="350" cy="378" r="11" />
        <circle className="dh-dna__node" cx="92" cy="534" r="11" />
        <circle className="dh-dna__node dh-dna__node--red" cx="428" cy="534" r="11" />
        <circle className="dh-dna__node" cx="145" cy="690" r="11" />
        <circle className="dh-dna__node dh-dna__node--red" cx="375" cy="690" r="11" />
      </g>
    </svg>
  );
}
    ...
    <div className="dantora-home" data-photos={JSON.stringify(visualAssets)}>
      ...
      <section className="dh-hero" aria-labelledby="dh-hero-title">
        <div className="dh-shell dh-hero__inner">
          ...
          <div className="dh-hero__scene" data-scene="hero" aria-hidden="true"><DnaBackdrop id="hero-dna" /></div>
      ...
      <section className="dh-contact" aria-labelledby="dh-contact-title">
        <div className="dh-contact__scene" data-scene="contact" aria-hidden="true"><DnaBackdrop id="contact-dna" /></div>
