...
body(.cm-root) > header.sticky{display!important}
html(.cm-root){scroll-behavior}

.cm-root{--green:#142F86;--green-deep:#DA1C29;--lime:#31B4F4;--lime-b(49 180 244 / .82);--mint:#FFFFFF;--mint-deep:#FFFFFF;--ink:#142F86;--muted(20 47 134 / .55);--subtle(20 47 134 / .72);--line(20 47 134 / .16);--glass:#FFFFFF;--glass-s:#FFFFFF;--glass-strong:#FFFFFF;
position;isolation;background:#FFFFFF;color(--ink);font-family(--font-lato),Arial,Helvetica,system-ui,sans-serif;font-weight:400;line-height:1.2;-webkit-font-smoothing;overflow-x}
.cm-root *{box-sizing}
.cm-root a(.cm-btn)(.cm-nav__contact)(.cm-svc__btn){color;text-decoration}
.cm-root button,.cm-root input,.cm-root textarea{font;color}
.cm-root ::selection{background(49 180 244 / .22);color(--green)}
.cm-shell{width(100% - 5rem,90rem);margin-inline}
@media(max-width:1023px){.cm-shell{width:100%}}
.cm-eyebrow{font-size:.875rem;font-weight:700;letter-spacing:.02em;text-transform;color(--green);margin:0}
.cm-lead{font-size(2.25rem,5.4vw,3.75rem);line-height:1.05;font-weight:700;letter-spacing:-.01em;margin:0}
.cm-lead__accent{display;font-size:1.25em;font-family(--font-playfair),Georgia,serif;font-style;font-weight:700;background(120deg,var(--green),var(--lime));-webkit-background-clip;background-clip;color}
.cm-lead__rest{display}
.cm-body{font-size:1rem;line-height:1.35;font-weight:400;margin:0}

/* buttons */
.cm-btn{display;align-items;justify-content;height:3rem;padding:0 2rem;border-radius:46px;border:1px solid transparent;font-size:1.25rem;font-weight:700;white-space;transition .15s,color .15s,transform .3s}
.cm-btn{transform(-2px)}
.cm-btn--pill{gap:.6rem}
.cm-btn--pill .cm-arrow{width:1.1rem;height:1.1rem}
.cm-hero__button{position;isolation;overflow;border-color;transition .9s ease,transform .3s}
.cm-hero__button::before{content:"";position;inset:0;z-index:0;transform(-105%);transition 1s cubic-bezier(.22,1,.36,1)}
.cm-hero__button>span,.cm-hero__button>.cm-arrow{position;z-index:1}
.cm-hero__button:hover::before,.cm-hero__button:focus-visible::before{transform(0)}
.cm-hero__button--primary{background:#142F86;color:#FFFFFF}
.cm-hero__button--primary::before{background(110deg,#142F86 0%,#31B4F4 52%,#142F86 100%)}
.cm-hero__button--secondary{background:#31B4F4;color:#142F86}
.cm-hero__button--secondary::before{background(110deg,#31B4F4 0%,#142F86 52%,#31B4F4 100%)}
.cm-hero__button--secondary,.cm-hero__button--secondary
.cm-btn--primary{background(--green);border-color(--green-deep);color:#FFFFFF}
.cm-btn--primary{background(--green-deep)}
.cm-btn--secondary{background(--lime);border-color(--lime-b);color:#142F86}
.cm-btn--secondary{background(--lime-b)}
.cm-btn--arrow{height:3.375rem;gap:.625rem;padding-right:.125rem;padding-left:2rem}
.cm-disc{display;place-items;width:3rem;height:3rem;flex;border-radius:999px;background(--lime);color:#142F86}
.cm-arrow{width:1rem;height:1rem;display}

/* generic section */
.cm-section{position}
.cm-panel{background(--mint)}
.cm-round-b{border-radius:0 0 48px 48px}
.cm-round-t{border-radius:48px 48px 0 0}

/* HERO */
.cm-hero{position;min-height:100lvh;overflow;padding:9rem 1.25rem 4rem}
@media(min-width:768px){.cm-hero{padding-inline:2.5rem}}
.cm-hero__inner{position;min-height(100lvh - 13rem);display;flex-direction}
.cm-hero__copy{position;z-index:3;display;flex-direction;gap:0;max-width:44.75rem;padding-top:2.5rem}
.cm-hero__head{display;flex-direction;gap:0}

/* rating badge */
.cm-rating{display;align-items;gap:.85rem;align-self;margin:0 0 1.35rem;text-decoration!important}
.cm-rating__faces{display;align-items}
.cm-rating__face{display;place-items;width:2.1rem;height:2.1rem;border-radius:999px;border:2px solid #FFFFFF;color:#FFFFFF;font-size:.8rem;font-weight:700;line-height:1}
...
.cm-rating__text{display;flex-direction;gap:.18rem}
.cm-rating__label{font-size:.9rem;font-weight:600;color(--ink)}
.cm-stars{position;display;line-height:0}
.cm-stars__row{display;gap:.06rem;color:#F58220}
.cm-stars__row svg{width:.95rem;height:.95rem;flex;fill;stroke;stroke-width:1.6}
.cm-stars__row--on{position;inset:0 auto 0 0;overflow;white-space}
.cm-stars__row--on svg{fill}
@media(min-width:1024px){.cm-rating{margin-bottom:1.6rem}.cm-rating__label{font-size:.95rem}}
.cm-hero__desc{max-width:18rem;margin-top:4.25rem;color(--subtle);border-left:2px solid var(--lime);padding-left:1rem}
.cm-hero__actions{display;flex-direction;flex-wrap;gap:.75rem;position;z-index:3;margin-top:4rem}
.cm-hero__actions .cm-btn{flex:0 1 10.5rem;width;height:2.875rem;padding:0 .85rem;font-size:.95rem}
.cm-hero__scene{position;top:1.5rem;right:0;left;width:52%;height:30rem;z-index:1;margin:0;pointer-events}
.cm-dna{display;width:100%;height:100%}
.cm-hero__meta{display;flex-direction;gap:1.5rem}
.cm-hero__rule{border:0;border-top:1px solid var(--line);margin:3rem 0 0}
.cm-hero__trust{display;align-items;gap:.75rem;font-size:1rem;color(--green);margin:1.25rem 0 0}
.cm-hero__trust span{height:1px;width:3.125rem;background(--green)}
.cm-chips{display;flex-wrap;gap:.25rem;margin:1.25rem 0 0;padding:0;list-style}
.cm-chips li{border:1px solid var(--line);background(--glass-s);-webkit-backdrop-filter(10px);backdrop-filter(10px);border-radius:37px;padding:.75rem 1.25rem;font-size:.875rem;font-weight:400}
@media(max-width:1023px){
.cm-hero__inner{width:100%}
.cm-hero__rule,.cm-hero__trust,.cm-chips{display}
.cm-hero__scene{top:-9rem;right:-1.25rem;bottom:-4rem;left:-1.25rem;width;height}
}
...
.cm-hero__inner{display;height:100%;min-height:0}
.cm-hero__copy{position;top:50%;left:2.5rem;transform(-50%);gap:0}
.cm-hero__head{gap:0}
.cm-hero__copy .cm-body{width:30.875rem}
.cm-hero__desc{max-width;margin-top:2.5rem}
.cm-hero__scene{position;inset-block:0;right;left:22.5rem;width:71.25rem;height;margin:0}
.cm-hero__actions{position;top;left;flex-direction;margin-top:2rem}
.cm-hero__actions .cm-btn{flex:0 0 10.5rem;width:10.5rem}
.cm-hero__rule{position;top:78.625%;right:2.5rem;left:2.5625rem;margin:0}
.cm-hero__trust{position;top:83.625%;left:2.5rem;margin:0}
.cm-chips{position;top:83.625%;left:55.5625rem;width:31.9375rem;justify-content;gap:.25rem;margin:0}
.cm-chips li{padding:1rem 2rem;font-size:1rem}
.cm-lead{font-size:3.75rem}
}

/* WHY */
.cm-why{min-height:100lvh;display;place-items;overflow;background:#FFFFFF;text-align;padding:6rem 1.25rem}
.cm-why__inner{position;z-index:2;display;flex-direction;align-items;gap:2rem;width(100% - 2.5rem,44.75rem)}
.cm-why .cm-lead{width:100%}
@media(min-width:1024px){.cm-why .cm-lead{width:39.25rem}.cm-why .cm-body{width:27.625rem}}
.cm-why__desc{max-width:44rem;color(--subtle);margin:0}
.cm-why__stats{display;flex-wrap;justify-content;align-items;gap:1.5rem 2.5rem;margin:0}
.cm-why__stat{display;align-items;gap:.9rem;text-align}
.cm-why__stat-icon{display;place-items;width:3.75rem;height:3.75rem;flex;border-radius:999px;background(49 180 244 / .12);color(--green)}
.cm-why__stat-icon svg{width:1.9rem;height:1.9rem}
.cm-why__stat-text{display;flex-direction;gap:.15rem}
.cm-why__stat dd{font-size:2rem;line-height:1;font-weight:700;margin:0;color(--green)}
.cm-why__stat dt{font-size:.9rem;font-weight:400;color(--subtle);margin:0;max-width:7rem}
@media(min-width:1024px){.cm-why__stats{gap:1.5rem 3.5rem}.cm-why__stat dd{font-size:2.5rem}}
@media(max-width:1023px){
.cm-why__stats{display;grid-template-columns(3,1fr);gap:1rem .5rem;width:100%}
.cm-why__stat{flex-direction;align-items;text-align;gap:.55rem}
.cm-why__stat-icon{width:3rem;height:3rem}
.cm-why__stat-icon svg{width:1.5rem;height:1.5rem}
.cm-why__stat-text{align-items;text-align}
.cm-why__stat dd{font-size:1.6rem}
.cm-why__stat dt{font-size:.78rem;max-width:6.5rem}
.cm-why__actions{width:100%;gap:.75rem}
.cm-why__actions .cm-btn{flex:1 1 0;width;min-width:0;height:2.875rem;padding:0 .75rem;font-size:.95rem}
}
.cm-why__actions{display;gap:1rem}
.cm-why__actions .cm-btn{width:11.25rem}
.cm-trail{position;inset:0;z-index:1;overflow;pointer-events}
.cm-trail__card{position;top:0;left:0;width:13.5rem;height:17rem;border-radius:24px;background-size;background-position;will-change,opacity;opacity:0}

/* WHY MAGNUS SLIDESHOW /
.cm-why-mag{position;z-index:10;background:#FFFFFF;padding:0}
.cm-why-mag__frame{position;overflow;width:100%;min-height:100lvh;background:#0B1533;isolation}
.cm-why-mag__shots{position;inset:-12% 0;transform(0,var(--cm-par,0),0);will-change}
...
the rule. */
.cm-why-mag__shot{position;inset:0;display;opacity:0;transform(1.05);transition 1.1s ease,transform 7s ease-out}
.cm-why-mag__shot img{display;width:100%;height:100%;object-fit;object-position}
.cm-why-mag__shot.is-on{opacity:1;transform(1)}

.cm-why-mag__body{position;z-index:2;display;flex-direction;justify-content;gap:2.5rem;min-height:100lvh;padding:2rem 1.25rem}
...
.cm-why-mag__copy{position;max-width:38rem}
.cm-why-mag__pane{animation .7s cubic-bezier(.2,0,0,1) both}
@keyframes cmWhyMagIn{from{opacity:0;transform(14px)}to{opacity:1;transform}}
/* heading and description share one font, size and line height; only the weight differs */
.cm-why-mag__title,.cm-why-mag__list li{font-family(--font-inter),var(--font-lato),system-ui,sans-serif;font-size:1.375rem;line-height:1.3;letter-spacing:-.01em;color:#FFFFFF;text-shadow:0 1px 12px rgb(8 16 38 / .58)}
.cm-why-mag__title{margin:0 0 1.35rem;font-weight:700}
.cm-why-mag__list{display;flex-direction;gap:.35rem;margin:0;padding:0;list-style}
...
.cm-why-mag__foot{position;display;flex-direction}
.cm-why-mag__tabs{display;grid-template-columns:1fr 1fr;gap:1.5rem}
.cm-root .cm-why-mag__tab{display;flex-direction;gap:.5rem;padding:0;border:0;background;text-align;cursor;color(255 255 255 / .82);transition .25s}
.cm-why-mag__tab.is-on,.cm-why-mag__tab,.cm-why-mag__tab
...
.cm-why-mag__rail{position;display;height:1px;background(255 255 255 / .55);overflow}
.cm-why-mag__fill{position;inset:-1px auto;left:0;width:0;background:#FFFFFF}
.cm-why-mag__tab.is-on .cm-why-mag__fill{animation linear forwards}
@keyframes cmWhyMagFill{from{width:0}to{width:100%}}
.cm-why-mag__cta{align-self;display;align-items;justify-content;min-height:2.75rem;margin-top:1.6rem;padding:0 1.75rem;border-radius:8px;background:#1B1B1B;color:#FFFFFF!important;font-size:.9375rem;font-weight:600;text-decoration!important;transition .2s}
.cm-why-mag__cta
@media(min-width:768px){.cm-why-mag__cta{margin-top:1.9rem}}

/* SERVICES /
.cm-services{position;background:#FFFFFF;padding:5rem 1.25rem 4rem}
@media(min-width:768px){.cm-services{padding-inline:2.5rem}}
.cm-services__intro{display;flex-direction;gap:1rem;max-width:56rem;margin:0 auto 2.5rem;text-align;align-items}
.cm-svc-grid{display;grid-template-columns:1fr;gap:1.25rem;width(100%,84rem);margin-inline}
@media(min-width:768px){.cm-svc-grid{grid-template-columns:1fr 1fr}}
.cm-svc{position;display;flex-direction;justify-content;min-height:26rem;overflow;border-radius:20px;background:#142F86}
@media(min-width:768px){.cm-svc{min-height:32rem}}
.cm-svc__img{position;inset:0;width:100%;height:100%;object-fit;transition .7s cubic-bezier(.2,0,0,1)}
.cm-svc .cm-svc__img{transform(1.05)}
.cm-svc--wf{justify-content}
.cm-svc--wf .cm-svc__head{order;width:52%;max-width:20rem;padding-right:0}
.cm-svc--wf .cm-svc__actions{order}
@media(max-width:1023px){
.cm-svc--wf .cm-svc__title{font-size:2rem;line-height:1.02}
}
@media(max-width:767px){
.cm-svc--wf .cm-svc__img{left:0;right;width:125%;object-position:50% 40%!important}
.cm-svc--wf .cm-svc__head{width:45%;max-width;padding-right:0}
...
.cm-svc__scrim{position;inset:0;background(90deg,rgb(0 0 0 / .72) 0%,rgb(0 0 0 / .5) 28%,rgb(0 0 0 / .2) 50%,rgb(0 0 0 / 0) 70%)}
.cm-svc__head{position;z-index:2;padding:1.75rem}
...
.cm-svc__title{font-family(--font-inter),var(--font-lato),system-ui,sans-serif;font-weight:800;letter-spacing:-.03em}
.cm-svc__list{display;flex-direction;align-items;gap:.5rem;margin:1.1rem 0 0;padding:0;list-style;max-width:26rem}
.cm-svc__list li{display;align-items;gap:.55rem;font-family(--font-inter),var(--font-lato),system-ui,sans-serif;font-size:1.0625rem;font-weight:400;line-height:1.2;color:#FFFFFF}
.cm-svc__ico{display;place-items;width:1.75rem;height:1.75rem;flex;border-radius:999px;background(255 255 255 / .18);color:#FFFFFF}
...
.cm-svc__actions{position;z-index:2;display;flex-direction;gap:.75rem;padding:1.75rem}
@media(min-width:520px){.cm-svc__actions{flex-direction}}
@media(min-width:768px){.cm-svc__actions{padding:2rem}}
.cm-svc__btn{display;flex:1 1 0;min-width:0;align-items;justify-content;gap:.5rem;height:3rem;padding:0 1.25rem;border-radius:999px;font-size:.95rem;font-weight:700;white-space;transition .2s,color .2s}
...
.cm-card__cta{position;z-index:2;display;align-items;justify-content}
.cm-card__cta p{margin:0;font-size:1rem}
.cm-card__ring{display;place-items;width:3.5rem;height:3.5rem;border-radius:999px;border:1px solid #142F86;color:#142F86}
@media(min-width:1024px){.cm-card__cta p{font-size:1.25rem}.cm-card__ring{width:4.5rem;height:4.5rem}}

/* ABOUT /
.cm-about{position;z-index:10;margin-top:0;background:#FFFFFF;padding:4rem 1.25rem}
@media(min-width:768px){.cm-about{padding-inline:2.5rem}}
.cm-about__banner{position;overflow;border-radius:24px;height:16rem}
.cm-about__banner img{position;inset:0;width:100%;height:100%;object-fit;object-position}
.cm-about__content{margin-top:2.5rem;display;flex-direction;gap:2.5rem}
.cm-about__stats{display;grid-template-columns:1fr 1fr;gap:2.5rem .625rem}
.cm-stat{display;flex-direction;gap:1rem;border-left:1px solid var(--green);padding-left:1rem}
...
.cm-about__para{font-size:1.375rem;line-height:1.2;font-weight:400;color(--ink);margin:0}
.cm-about__para .accent{font-family;font-style;font-weight:700;color:#0A1633}
.cm-about__right{display;flex-direction;gap:2rem}
.cm-about__actions{display;flex-wrap;gap:1rem}
@media(min-width:768px){.cm-about__banner{height:24rem}.cm-about__para{font-size:1.75rem}}
@media(min-width:1024px){
.cm-about{height:71.5rem;padding:0}
.cm-about__banner{position;top:2.5rem;left:2.5rem;width:85rem;height:31.25rem}
.cm-about__content{position;top:37.75rem;left:2.375rem;width:85.125rem;margin:0;flex-direction;align-items;justify-content;gap:0}
.cm-about__left{display;flex-direction;gap:5.5rem;width:28rem}
...
.cm-about__right{display;flex-direction;gap:8.375rem;width:49.3125rem}
.cm-about__para{font-size:2.25rem}
.cm-about__actions .cm-btn
.cm-about__actions .cm-btn
}
.cm-about__actions .cm-btn--primary{color:#FFFFFF;border-color(--green)}
.cm-about__actions .cm-btn--primary{background(--green);color:#FFFFFF}

/* TESTIMONIALS */
.cm-testimonials{position;z-index:10;background:#F5F8FB;color:#16233B;padding:4.5rem 1.5rem 5.5rem}
.cm-testimonials__inner{max-width:75rem;margin:0 auto}
.cm-testimonials__head{max-width:35rem;margin:0 auto 3rem;text-align}
.cm-testimonials__eyebrow{display;align-items;gap:.375rem;font-size:.8125rem;font-weight:600;letter-spacing:.02em;color:#1F5F8B;background:#EAF2F8;padding:.375rem .875rem;border-radius:999px;margin-bottom:1.125rem}
.cm-testimonials__title{font-size(1.625rem,3.4vw,2.25rem);line-height:1.15;margin:0 0 .75rem;letter-spacing:-.01em;font-weight:700}
.cm-testimonials__head p{margin:0;color:#4A5872;font-size:.96875rem;line-height:1.5}
.cm-testimonials__marquee{position;overflow;-webkit-mask-image(to right,transparent 0,#000 3rem,#000 calc(100% - 3rem),transparent 100%);mask-image(to right,transparent 0,#000 3rem,#000 calc(100% - 3rem),transparent 100%)}
.cm-testimonials__track{display;gap:1.25rem;width;padding:.5rem .25rem 1.5rem;margin:0;list-style;animation 42s linear infinite}
.cm-testimonials__marquee .cm-testimonials__track,.cm-testimonials__marquee .cm-testimonials__track{animation-play-state}
@keyframes cmReviewsScroll{from{transform(0)}to{transform(-50%)}}
.cm-review-card{flex:0 0 21.25rem;background:#FFFFFF;border:1px solid #E3E9F1;border-radius:16px;box-shadow:0 1px 2px rgb(22 35 59 / .04),0 8px 24px -12px rgb(22 35 59 / .12);padding:1.375rem 1.375rem 1.25rem;display;flex-direction;gap:.875rem;scroll-snap-align}
.cm-review-card__top{display;align-items;gap:.75rem}
.cm-review__avatar{display;place-items;width:2.75rem;height:2.75rem;border-radius:50%;flex-shrink:0;background:#EAF2F8;color:#FFFFFF;font-size:.8125rem;font-weight:700;letter-spacing:.01em}
...
.cm-review__name{margin:0;font-size:.90625rem;font-weight:600;white-space;overflow;text-overflow}
.cm-review__profile{margin:.125rem 0 0;font-size:.75rem;color:#68748A;white-space;overflow;text-overflow}
.cm-review__meta{margin:.35rem 0 0;font-size:.78125rem;color:#4A5872;display;align-items;gap:.375rem}
.cm-testimonials__eyebrow-stars,.cm-review__stars{display;align-items;gap:.0625rem;color:#E0A13C}
.cm-testimonials__eyebrow-stars svg{width:.6875rem;height:.6875rem;fill;stroke;stroke-width:1.5}
.cm-review__stars svg{width:.75rem;height:.75rem;fill;stroke;stroke-width:1.5}
.cm-review__stars svg.is-filled{fill}
.cm-review__google{flex-shrink:0;opacity:.9}
.cm-review__quote{margin:0;font-size:.90625rem;line-height:1.55;color:#16233B;white-space}
.cm-review__quote.is-clamped{display:-webkit-box;-webkit-line-clamp:4;-webkit-box-orient;overflow;white-space}
.cm-review__actions{display;flex-direction;align-items;gap:.625rem;margin-top}
.cm-root .cm-review__more{align-self;border:0;background;color:#1F5F8B;font-size:.84375rem;font-weight:600;padding:0;cursor;font-family}
.cm-review__more,.cm-review__more
.cm-review__google-link{display;align-items;color:#1F5F8B!important;font-size:.8125rem;font-weight:600;text-decoration!important;text-underline-offset:3px}
.cm-review__google-link,.cm-review__google-link
.cm-testimonials .sr-only{position;width:1px;height:1px;padding:0;margin:-1px;overflow;clip(0,0,0,0);white-space;border:0}
...
/* CONTACT */
.cm-contact{position;z-index:20;min-height:100lvh;overflow;background:#FFFFFF;padding:5rem 1.25rem}
@media(min-width:768px){.cm-contact{padding-inline:2.5rem}}
.cm-contact__layout{display;flex-direction;gap:2rem}
.cm-contact__location{display;flex-direction;gap:1.5rem;min-width:0}
.cm-contact__head{display;flex-direction;gap:1.5rem}
.cm-contact__title{font-size(2.25rem,5.4vw,3.75rem);line-height:1.05;font-weight:700;margin:0}

.cm-location__map{position;overflow;width:100%;height:22rem;border:1px solid var(--line);border-radius:24px;background:#EAF2F8}
.cm-location__map iframe{display;width:100%;height:100%;border:0}
.cm-location__link{position;left:1rem;bottom:1rem;display;align-items;justify-content;min-height:2.75rem;padding:0 1.25rem;border-radius:999px;background:#142F86;color:#FFFFFF!important;font-size:.875rem;font-weight:700;text-decoration!important;box-shadow:0 8px 24px rgb(20 47 134 / .22)}
.cm-location__link
.cm-form{margin-top:0;display;flex-direction;gap:2rem;border:1px solid var(--line);border-radius:24px;background:#FFFFFF;padding:1.5rem}
.cm-form__mark{display;align-items}
.cm-form__mark img{display;width;height:2.5rem;max-width:100%;object-fit;object-position center}
...
.cm-form form{display;flex-direction;gap:1.5rem}
.cm-field{display;flex-direction;gap:1rem}
.cm-field label{font-size:.875rem;font-weight:700;text-transform;color(--green)}
.cm-field input,.cm-field textarea{width:100%;border:1px solid var(--line);background:#FFFFFF;border-radius:8px;padding:1.25rem 2rem;font-weight:400;outline}
.cm-field input::placeholder,.cm-field textarea::placeholder{color(--subtle)}
.cm-field input,.cm-field textarea{border-color(--green)}
.cm-field textarea{resize;min-height:5.5rem}
.cm-form__consent{font-size:1rem;font-weight:400;color(--subtle);margin:0}
.cm-form__consent a{color(--ink);text-decoration}
/* The label is forced white with !important: the button reset above sets
color, and the navy button background makes any inherited colour
...
.cm-root .cm-form__submit{display;align-items;justify-content;height:3.375rem;border:1px solid var(--green-deep);background(--green);color:#FFFFFF!important;border-radius:46px;padding:0 .125rem 0 2rem;font-size:1.25rem;font-weight:700;cursor;transition .15s}
.cm-root .cm-form__submit .cm-form__submit-label{color:#FFFFFF!important;-webkit-text-fill-color:#FFFFFF}
.cm-root .cm-form__submit{background(--green-deep)}
.cm-root .cm-form__submit .cm-form__submit-label{color:#FFFFFF!important;-webkit-text-fill-color:#FFFFFF}
@media(min-width:768px){.cm-form{padding:2rem}}
@media(min-width:1024px){
.cm-contact{padding:2.5rem}
.cm-contact__layout{display;grid-template-columns(0,1fr) 35.0625rem;gap:2rem;min-height(100lvh - 5rem);align-items}
.cm-contact__location{gap:2rem}
.cm-contact__head{gap:1.5rem}
.cm-contact__title{font-size:3.75rem}
.cm-location__map{flex:1;height;min-height:22rem}
.cm-form{margin:0;gap:2rem}
.cm-form form{flex:1}
.cm-form__submit{margin-top}
}

/* reveals */
.cm-root .cm-reveal .cm-word{display;overflow;vertical-align}
.cm-root .cm-reveal .cm-word>span{display;transform(110%);transition .8s cubic-bezier(.2,0,0,1);transition-delay(var(--w,0)*40ms)}
.cm-root.is-ready .cm-reveal.in .cm-word>span{transform(0)}
.cm-root [data-rise]{opacity:0;transform(28px);transition .8s ease,transform .8s cubic-bezier(.2,0,0,1)}
.cm-root.is-ready [data-rise].in{opacity:1;transform}

/* preloader /
.cm-pre{position;inset:0;z-index:90;display;place-items;background:#FFFFFF;color:#142F86;transition .9s cubic-bezier(.76,0,.24,1),visibility 0s .9s;animation .01s 1.25s forwards}
.cm-root.is-ready .cm-pre{transform(-100%);visibility}
/ Brand logo, not a spinning glyph — a wordmark must stay upright, so it
breathes instead of rotating. */
.cm-pre__mark{display;width;height;max-width(17rem,62vw);max-height:5rem;object-fit;animation 1.5s ease-in-out infinite}
@keyframes cmPreBreathe{0%,100%{opacity:.5;transform(.97)}50%{opacity:1;transform(1)}}
@keyframes cmPreFail{to{visibility;pointer-events;transform(-100%)}}

@media(prefers-reduced-motion){
html(.cm-root){scroll-behavior}
.cm-root .cm-reveal .cm-word>span,.cm-root [data-rise]{transition!important;transform!important;opacity:1!important}
.cm-pre{display}
.cm-svc__img{transition}
.cm-about{margin-top:0}
.cm-hero__scene,.cm-trail{display}
.cm-testimonials__marquee{overflow-x;-webkit-mask-image;mask-image}
.cm-testimonials__track{animation;scroll-snap-type proximity}
.cm-why-mag__shots{transform}
.cm-why-mag__shot{transition;transform}
.cm-why-mag__pane{animation}
.cm-why-mag__tab.is-on .cm-why-mag__fill{animation;width:100%}
.cm-why-mag__cta
}
`;
...
function spring(v){return {v,t,vel:0,step(dt,k,d){const a=(this.t-this.v)k-this.veld;this.vel+=adt;this.v+=this.veldt;return this.v;}};}
...
if(burger&&menu){
const toggle=v=>{ const open=v!==undefined?v.dataset.open!=='true'; menu.dataset.open=open?'true':'false'; burger.setAttribute('aria-expanded',open?'true':'false'); burger.setAttribute('aria-label',open?'Close menu':'Open menu'); if(!open)menu.querySelectorAll('details[open]').forEach(d=>d.removeAttribute('open')); };
...
const active=document.activeElement; const item=active&&active.closest?active.closest('.cm-nav__item');
...
const U={uTime:{value:0},uAppear:{value:0},uHelixA:{value THREE.Vector3().fromArray(hx(CFG.helixColorA))},uHelixB:{value THREE.Vector3().fromArray(hx(CFG.helixColorB))},uInkCore:{value THREE.Vector3().fromArray(hx(CFG.inkCore))},uInkMid:{value THREE.Vector3().fromArray(hx(CFG.inkMid))},uInkEdge:{value THREE.Vector3().fromArray(hx(CFG.inkEdge))},uHelixSize:{value.helixSize},uInkSize:{value.inkSize},uBrightness:{value.brightness},uHelixOpacity:{value.helixOpacity},uInkOpacity:{value.inkOpacity},uInkGrow:{value.inkGrow},uRadius:{value.radius},uHeight:{value.height},uTwist:{value.twist},uThick:{value.strandThick},uWave:{value.wave},uEmitRate:{value.emitRate},uSpread:{value.spread},uRise:{value.rise},uTurb:{value.turbulence},uNoiseFreq:{value.noiseFreq},uNoiseEvolve:{value.noiseEvolve},uCursor:{value THREE.Vector3()},uRepelRadius:{value.pointerRadius},uRepelStrength:{value.pointerStrength},uActivity:{value:0},uPixelScale:{value:1}};
function seedGeo(count){ const g=new THREE.BufferGeometry(); const a=new Float32Array(count3); for(let i=0;i<a.length;i++)a[i]=Math.random()64; g.setAttribute('position',new THREE.BufferAttribute(a,3)); return g; }
function mat(vs,fs){ return new THREE.ShaderMaterial({uniforms,vertexShader,fragmentShader,transparent,depthWrite,depthTest,blending.MultiplyBlending,premultipliedAlpha}); }
...
function setSize(w,h){ if(w<=0||h<=0)return; W=w;H=h; const pr=Math.min(window.devicePixelRatio,CFG.maxPixelRatio); renderer.setPixelRatio(pr); renderer.setSize(w,h,false); cam.aspect=w/h; cam.updateProjectionMatrix(); U.uPixelScale.value=(hpr)/REF_H; }
...
if(fine){ window.addEventListener('pointermove',onMove,{passive}); document.documentElement.addEventListener('mouseleave',onLeave); }
let lastW=0,lastH=0;
...
function render(now){ const wc=now/1000; const dt=Math.min(MAXD,wc-lastF); lastF=wc; elapsed+=dt; const pAlpha=1-Math.pow(1-EASE_P,dt60); pE.x+=(pT.x-pE.x)pAlpha; pE.y+=(pT.y-pE.y)pAlpha; U.uTime.value=elapsed; spin.rotation.y=0; cam.position.x=pE.xCFG.parallax; cam.position.y=pE.yCFG.parallax; cam.position.z=CFG.camDist; cam.lookAt(0,0,0); cursorTarget.set(0,0,0); if(active){ ndc.set(pE.x,pE.y,0.5).unproject(cam); ray.copy(ndc).sub(cam.position).normalize(); const dz=ray.z; if(Math.abs(dz)>1e-4){ const d=-cam.position.z/dz; if(d>0&&isFinite(d)) cursorTarget.copy(cam.position).addScaledVector(ray,d); } } cursorWorld.lerp(cursorTarget,1-Math.pow(1-EASE_C,dt60)); const idle=(now-lastMove)/1000; const want=(active&&idle<IDLE)?1:0; activity+=(want-activity)(1-Math.pow(1-EASE_A,dt*60)); U.uCursor.value.copy(cursorWorld); U.uActivity.value=activity; const sm=(now-started)/1000; U.uAppear.value=Math.max(0,Math.min(1,(sm-A_DELAY)/A_DUR)); renderer.render(scene,cam); }
...
const watch=()=>{ if(boot()||state.mo)return; state.mo=new MutationObserver(boot); state.mo.observe(document.documentElement,{childList,subtree}); };
