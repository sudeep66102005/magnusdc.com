      ...
      <Section>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {categoryOrder.map((category) => {
            ...
            return (
              <Link key={category} href={`/health-packages/${category}`} className="group rounded-[1.5rem] border border-[#142F86]/18 bg-white p-6 transition duration-300 hover:-translate-y-1 hover:border-[#31B4F4]/60 hover:shadow-[0_20px_50px_-32px_rgba(20,47,134,0.65)]">
                <span className="flex size-10 items-center justify-center rounded-xl bg-[#31B4F4]/12 text-[#142F86]"><HeartPulse className="size-4" /></span>
                <p className="mt-5 text-lg font-black text-[#142F86]">{details.label}</p>
                <p className="mt-2 text-sm leading-6 text-[#142F86]/70">{details.description}</p>
        ...
        <div className="mt-10 grid gap-4 rounded-[2rem] bg-[#142F86] p-6 text-white md:grid-cols-[1fr_auto] md:items-center md:p-8">
          ...
          <a href={`${basePath}/assets/logo/Clarus%20Magnus%20Packages%2015.12.2025.pdf`} target="_blank" rel="noreferrer" className="inline-flex h-11 items-center justify-center gap-2 rounded-full bg-white px-5 text-sm font-black text-[#142F86] transition hover:bg-[#31B4F4]/12">
