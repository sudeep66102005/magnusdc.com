...
export function PackageCard({ pkg }: { pkg: HealthPackageContent }) {
  return (
    <article className="flex h-full flex-col rounded-[1.75rem] border border-[#142F86]/18 bg-white p-7 transition duration-300 hover:-translate-y-1 hover:border-[#31B4F4]/55 hover:shadow-[0_24px_60px_-36px_rgba(20,47,134,0.65)]">
      ...
      <p className="mt-3 text-sm leading-6 text-[#142F86]/70">{pkg.summary}</p>
      <ul className="mt-6 space-y-2.5">
        {pkg.testsIncluded.slice(0, 6).map((test) => (
          <li key={test} className="flex items-start gap-2.5 text-sm leading-5 text-[#142F86]/82">
      ...
      <div className="mt-auto flex items-end justify-between gap-4 border-t border-[#142F86]/14 pt-6">
        <div>
          {pkg.mrp && <p className="text-xs font-bold text-[#142F86]/55 line-through">MRP ₹{pkg.mrp.toLocaleString("en-IN")}</p>}
